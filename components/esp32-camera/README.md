...
```
cd $project\lib
git submodule add -b master https://github.com/espressif/ESP32-CAM PSRAMera.git
```

Then in `platformio.ini` file
```
build_flags =
   -I../lib/ESP32-CAM PSRAMera
```
...
## Platform.io Kconfig 
Kconfig is used by the platform.io menuconfig (accessed by running: `pio run -t menuconfig`) to interactively manage the various #ifdef statements throughout the espidf and supporting libraries (i.e. this repo: ESP32-CAM PSRAMera and arduino-esp32.git).  The menuconfig process generates the `sdkconfig` file which is ultimately used behind the scenes by espidf compile+build process. 
...
esp_err_t http_send_jpg_handler(httpd_req_t *req){
    camera_fb_t * fb = NULL;
